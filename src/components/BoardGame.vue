<template>
    <div class="board">
        <div class="frame-wrapper" :style="frameSize">
            ...
            <div class="original" v-if="showingOriginal && image" @click="showingOriginal = false"
                :style="{ background: `url(${image})` }">
            </div>
            <div class="frame" :style="frameSize">
                <Tile v-for="tile in tiles" :key="tile.position" :tile="tile" @moving="moveTile" ref="tiles" />
            </div>
        </div>
...
export default {
    ...
    methods: {
        start({ image, size }) {
            ...
            const img = new Image();
            img.onload = () => {
                //console.log(img.width, size.horizontal)
                // this.tileSize.width = Math.floor(250)
                // this.tileSize.height = Math.floor(300)
